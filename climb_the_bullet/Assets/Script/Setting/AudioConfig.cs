...
public class AudioConfig : MonoBehaviour
{
    ...
    void Awake()
    {
        float bgmValue = PlayerPrefs.GetFloat("bgmSliderPosition", 0f); //第一引数にセットする場所、第二引数はなかった時の数値
        float seValue = PlayerPrefs.GetFloat("seSliderPosition", 0f); //第一引数にセットする場所、第二引数はなかった時の数値
