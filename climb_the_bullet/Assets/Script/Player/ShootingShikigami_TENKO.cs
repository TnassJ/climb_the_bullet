...
public class ShootingShikigami_TENKO : MonoBehaviour
{
    //[SerializeField]
    ...
    public Shot_Shikigami ShotPrefab_Shikigami_TENKO; // 弾のプレハブ
    ...
    [SerializeField]
    private List<TENKOShotData> shotData;

    public AudioClip ShotClip_TENKO; // 弾のSE
    ...
    void Update()
    {
        ...
        audioSource.PlayOneShot(ShotClip_TENKO, 0.04f); // SE再生
    }
    ...
    private void ShootNWay(float angleBase, float angleRange, float speed, int count)
    {
        ...
        if (1 < count)
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                // 発射する弾を生成する
                var shot = Instantiate(ShotPrefab_Shikigami_TENKO, pos, rot);
