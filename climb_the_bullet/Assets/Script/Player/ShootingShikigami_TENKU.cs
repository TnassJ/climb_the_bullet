...
public class ShootingShikigami_TENKU : MonoBehaviour
{
    //[SerializeField]
    ...
    public Shot_Shikigami ShotPrefab_Shikigami_TENKU; // 弾のプレハブ
    ...
    [SerializeField]
    private List<TENKUShotData> shotData;

    public AudioClip ShotClip_TENKU; // 弾のSE
    ...
    private void ShootNWay(float angleBase, float angleRange, float speed, int count)
    {
        ...
        if (1 < count)
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                ...
                // 発射する弾を生成する
                var shot = Instantiate(ShotPrefab_Shikigami_TENKU, pos, rot);
